    ...
    public function index()
{
    ...
    $archivosSubidos = $controles->where('documentoEvidencia')->where('idEtapaTecnologica', 7)->count();
    $archivosFaltantes = 13 - $archivosSubidos;// Cambia 100 por el número total esperado de archivos
    return view('controlminimo.equipocomputo.index', compact('controles', 'controlCompleto', 'controlIncompleto', 'porcentajeCumplimiento','archivosSubidos','archivosFaltantes'));
    
}
    public function create(Request $request)
    {
        return view ('controlminimo.equipocomputo.create');
    }
    public function store(Request $request)
    {
        ...
        $controlminimo->idEtapaTecnologica = 7;
        $user = auth()->id();
        ...
        $controlminimo->save();
        return redirect()->route('equipo-computo.index');
    ...
    public function edit(string $id){
        ...
        return view('controlminimo.equipocomputo.edit', compact('control'));
    }
    public function update(Request $request, $id){
        $control = ControlMinimo::find($id);
        $control->idEtapaTecnologica = 7;
        $user = auth()->id();
        ...
        $control->save();
        return redirect()->to(url('/equipo-computo'));
    }
    ...
    $archivosFaltantes = 13 - $archivosSubidos;// Cambia 100 por el número total esperado de archivos
        ...
        $controlesFiltrados = ControlMinimo::where('semestre', $semestreSeleccionado)->where('idEtapaTecnologica', 7)->get();

        return view('controlminimo.equipocomputo.index', ['controles' => $controlesFiltrados], compact('controles', 'controlCompleto', 'controlIncompleto', 'porcentajeCumplimiento', 'archivosSubidos', 'archivosFaltantes'));
    }
