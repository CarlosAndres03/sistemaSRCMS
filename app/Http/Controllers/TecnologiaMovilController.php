...
class TecnologiaMovilController extends Controller
{
    public function index()
{
    $controles = DB::table('ControlMinimos')
        ->where('idEtapaTecnologica', 8)
        ->get();
    ...
    $archivosSubidos = $controles->where('documentoEvidencia')->where('idEtapaTecnologica', 8)->count();
    $archivosFaltantes = 3 - $archivosSubidos;// Cambia 100 por el número total esperado de archivos
    return view('controlminimo.tecmovil.index', compact('controles', 'controlCompleto', 'controlIncompleto', 'porcentajeCumplimiento','archivosSubidos','archivosFaltantes'));
    
}
    public function create(Request $request)
    {
        return view ('controlminimo.tecmovil.create');
    }
    public function store(Request $request)
    {
        ...
        $controlminimo->idEtapaTecnologica = 8;
        $user = auth()->id();
        ...
        $controlminimo->save();
        return redirect()->route('tec-movil.index');
    ...
    public function edit(string $id){
        ...
        return view('controlminimo.tecmovil.edit', compact('control'));
    }
    public function update(Request $request, $id){
        $control = ControlMinimo::find($id);
        $control->idEtapaTecnologica = 8;
        $user = auth()->id();
        ...
        $control->save();
        return redirect()->to(url('/tec-movil'));
    }
    ...
    $archivosFaltantes = 3 - $archivosSubidos;// Cambia 100 por el número total esperado de archivos
        ...
        $controlesFiltrados = ControlMinimo::where('semestre', $semestreSeleccionado)->where('idEtapaTecnologica', 8)->get();

        return view('controlminimo.tecmovil.index', ['controles' => $controlesFiltrados], compact('controles', 'controlCompleto', 'controlIncompleto', 'porcentajeCumplimiento', 'archivosSubidos', 'archivosFaltantes'));
    }
