...
class EquiposFisicosController extends Controller
{
    public function index()
{
    $controles = DB::table('ControlMinimos')
        ->where('idEtapaTecnologica', 4)
        ->get();
    ...
    $archivosSubidos = $controles->where('documentoEvidencia')->where('idEtapaTecnologica', 4)->count();
    $archivosFaltantes = 3 - $archivosSubidos;// Cambia 100 por el número total esperado de archivos
    return view('controlminimo.equiposfisicos.index', compact('controles', 'controlCompleto', 'controlIncompleto', 'porcentajeCumplimiento','archivosSubidos','archivosFaltantes'));
    
}
    public function create(Request $request)
    {
        return view ('controlminimo.equiposfisicos.create');
    }
    public function store(Request $request)
    {
        ...
        $controlminimo->idEtapaTecnologica = 4;
        $user = auth()->id();
        ...
        $controlminimo->save();
        return redirect()->route('equipos-fisicos.index');
    ...
    public function edit(string $id){
        ...
        return view('controlminimo.equiposfisicos.edit', compact('control'));
    }
    public function update(Request $request, $id){
        $control = ControlMinimo::find($id);
        $control->idEtapaTecnologica = 4;
        $user = auth()->id();
        ...
        $control->save();
        return redirect()->to(url('/equipos-fisicos'));
    }
    ...
    $archivosFaltantes = 3 - $archivosSubidos;// Cambia 100 por el número total esperado de archivos
        ...
        $controlesFiltrados = ControlMinimo::where('semestre', $semestreSeleccionado)->where('idEtapaTecnologica', 4)->get();

        return view('controlminimo.equiposfisicos.index', ['controles' => $controlesFiltrados], compact('controles', 'controlCompleto', 'controlIncompleto', 'porcentajeCumplimiento', 'archivosSubidos', 'archivosFaltantes'));
    }
