...
class RedesTelController extends Controller
{
    public function index()
{
    $controles = DB::table('ControlMinimos')
        ->where('idEtapaTecnologica', 6)
        ->get();
    ...
    $archivosSubidos = $controles->where('documentoEvidencia')->where('idEtapaTecnologica', 6)->count();
    $archivosFaltantes = 17 - $archivosSubidos;// Cambia 100 por el número total esperado de archivos
    return view('controlminimo.redestelecomunicaciones.index', compact('controles', 'controlCompleto', 'controlIncompleto', 'porcentajeCumplimiento','archivosSubidos','archivosFaltantes'));
    
}
    public function create(Request $request)
    {
        return view ('controlminimo.redestelecomunicaciones.create');
    }
    public function store(Request $request)
    {
        ...
        $controlminimo->idEtapaTecnologica = 6;
        $user = auth()->id();
        ...
        $controlminimo->save();
        return redirect()->route('red-tel.index');
    ...
    public function edit(string $id){
        ...
        return view('controlminimo.redestelecomunicaciones.edit', compact('control'));
    }
    public function update(Request $request, $id){
        $control = ControlMinimo::find($id);
        $control->idEtapaTecnologica = 6;
        $user = auth()->id();
        ...
        $control->save();
        return redirect()->to(url('/red-tel'));
    }
    ...
    $archivosFaltantes = 17 - $archivosSubidos;// Cambia 100 por el número total esperado de archivos
        ...
        $controlesFiltrados = ControlMinimo::where('semestre', $semestreSeleccionado)->where('idEtapaTecnologica', 6)->get();

        return view('controlminimo.redestelecomunicaciones.index', ['controles' => $controlesFiltrados], compact('controles', 'controlCompleto', 'controlIncompleto', 'porcentajeCumplimiento', 'archivosSubidos', 'archivosFaltantes'));
    }
